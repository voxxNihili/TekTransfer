...
const Setting = (props) => {
    ...
    const settingData = async () => {
        await axios
            .get(`/api/order/setting/${params.id} `, {
                headers: {
                    Authorization:
    ...
    const handleSubmit = (values, { resetForm }) => {
        ...
        axios
            .post("/api/order/setting", data, config)
            .then((res) => {
                if (res.data.success) {
                    goToLoginPage();
                } else {
                    ...
                    <Formik
                        ...
                        validationSchema={Yup.object().shape({
                            customerCode: Yup.string().required(
                                "Müşteri Cari Kodu Adı Zorunludur"
                            ),
                            customerType: Yup.number().required(
                                "Müşteri Cari Türü Zorunludur"
                            ),
                            companyId: Yup.number().required(
                                "Firma Idsi Zorunludur"
                            ),
                            companyName: Yup.string().required(
                                "Firma Adı Zorunludur"
                            ),
                            sqlPeriod: Yup.string().required(
                                "Logo Dönem Zorunludur"
                            ),
                            sqlCompanyId: Yup.string().required(
                                "Logo Sql Firma Zorunludur"
                            ),
                        })}
                    >
                        {({
